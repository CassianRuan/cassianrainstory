...
- 首次点击“开始故事”时会解锁浏览器音频并立即启动当前场景环境音。
- 剧情会自动朗读并推进；可以暂停、继续或跳过当前朗诵。朗诵期间雨声自动降到正常音量的 55%，结束后平滑恢复。
- 左上角“故事”按钮可以回到已解锁节点，未解锁节点不能越级进入。
- 右上角“设置”可以分别调整朗诵、音效和音乐音量，并为角色选择本机声音。
- 找不同、点击线索、钢琴 QTE 与 2D 俯视公路驾驶均限时 60 秒、最多 3 次错误；驾驶时使用方向键或 A/D 快速换道，碰撞后短暂减速并闪烁无敌，障碍间隔从约 2.5 秒逐渐缩短到约 1.8 秒。失败后可无限重试。桥梁答题必须选择正确答案才能继续。
